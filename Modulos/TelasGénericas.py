...
def TelaEntradaDeTexto(tela, estados, eventos, parametros):
    ...
    Info1 = parametros.get("TelaEntradaDeTexto", {}).get("Info1")
    ...
    Botao(
        ...
        lambda: [Enviar(parametros["TelaEntradaDeTexto"]["Texto"]) if Info1 is None else Enviar(parametros["TelaEntradaDeTexto"]["Texto"], Info1)] + [voltar()],
...
def TelaDeCerteza(tela, estados, eventos, parametros):
    ...
    Texturas["AzulRoxa"], Cores["preto"], Cores["branco"],
    lambda: (
        [f() for f in Funcao] if isinstance(Funcao, list) else [Funcao()]
    ) + ([Voltar()] if Voltar else []),
    ...
    Botao(
        ...
        Texturas["AzulRoxa"], Cores["preto"], Cores["branco"],
        lambda: Voltar() if Voltar else None,
